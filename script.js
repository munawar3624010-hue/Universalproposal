...
function showRing(x,y){
    ...
    initials.innerText = "R ❤ E"; // Change initials here
    ring.appendChild(initials);
    document.body.appendChild(ring);
    setTimeout(()=>ring.remove(), 3000);
}

function startRoseRain(count=100){
...
function showHandwriting(name="Ghuriya"){
...
function starsFormName(text="Rajkumari"){
...
function secretYes(){
    ...
    btn.onclick = e => {
        ...
        startRoseRain(250);
        showHandwriting("I Miss You Esha");
...
function doubleBlackHoleTransition() {
    ...
    setTimeout(() => {
        createCinematicBlackHole(x, y);
        heartExplosion(x, y);
        showHandwriting("THANKS To Loves Me😭✨");
    }, 2000);
}
